...
No nosso caso, um sensor com acelerômetros e giroscópios de três eixos \(xyz\) em \(frd\), fixo no centro de massa do robô, temos que o acelerômetro nos apresenta uma leitura de \( \dot{U}, \dot{V}, \dot{W}\), e o giroscópio apresenta uma leitura de \( P, Q, R\). Esses valores compõem o vetor \(\mathbf{u}\).
...
Para nosso sensor com acelerômetros e giroscópios de três eixos, as leituras são simultâneas e obtidas em intervalos de tempo regulares \(Ts\), que podemos escolher entre algumas opções, obtendo:
...
Pelo método de Euler, podemos obter a atitude se consideramos:
\begin{align*}
{\Phi}{\left[ k \right]} 
...
A pretensão é chegar em algo assim:
\begin{align*}
...
\begin{align*}
    {\Phi} \left(t\right) &=
        \int_{t_{0}}^{t} H \left( \Phi \right) {\mathbf{\omega}^{frd}_{b/e}}\left(t\right) \textrm{d}t \\
    {^{e}{\mathbf{p}^{tp}_{cm/Q}}\left(t\right)} 
        &= \int_{t_0}^{t} {^{e}{\dot{\mathbf{p}}^{tp}_{cm/Q}}\left(t\right)} \\
        &= \int_{t_0}^{t}{C_{tp/frd}\left({\Phi}\right)}{\mathbf{v}^{frd}_{cm/e}\left(t\right)}\textrm{d}t \\
        &= \int_{t_0}^{t}{C_{tp/frd}\left({\Phi}\right)}
        {\left(
        \int_{t_0}^{t} \textstyle{\frac{1}{m}} {\mathbf{F}^{frd}}\left(t\right)
        + {C_{frd/tp}\left(\Phi\right)}{\mathbf{g}^{tp}}
        - {\tilde{\mathbf{\omega}}^{frd}_{b/e}\left(t\right)} {\mathbf{v}^{frd}_{cm/e}\left(t\right)} \textrm{d}t
\right)} \textrm{d}t
\end{align*}

com condições iniciais, para o instante \(t_{0} = 0\):
\begin{align*}
    {\Phi} \left[0\right] 
