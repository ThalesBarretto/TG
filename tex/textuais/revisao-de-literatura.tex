...
A velocidade também pode ser obtida de um sistema fixo no corpo. Mudando as derivadas na equação (1.7--13), com \(\mathbf{G} \equiv \mathbf{g}\), temos~\cite{Stevens2016}:
...
\begin{figure}[H]
    ...
    \fonte{\citeonline{Stevens2016}}
\end{figure}

No sensor com acelerômetros e giroscópios de três eixos \(xyz\) em \(frd\), o acelerômetro nos apresenta uma leitura de \( \dot{U}, \dot{V}, \dot{W}\), e o giroscópio uma leitura de \( P, Q, R\). As leituras são simultâneas e obtidas em intervalos de tempo regulares \(Ts\), que podemos escolher entre algumas opções. De um modo geral:
...
\begin{align*}
    &{^{b}{\dot{\mathbf{v}}}^{frd}_{cm/e}_\left[k\right]}
        ...
        \text{ (leitura do acelerômetro no instante \(t=kT\))}, \\
     &\mathbf{\omega}^{frd}_{b/e}_{\left[k\right]}
        \equiv \begin{bmatrix} P_{\left[k\right]} & Q_{\left[k\right]} & R_{\left[k\right]} \end{bmatrix}^{T}
        \text{ (leitura do giroscópio no instante \(t=kT\))} \\
\end{align*}
%\begin{align*}
%    {\mathbf{\omega}^{frd}_{b/e}} &= \begin{bmatrix} P & Q & R \end{bmatrix}^{T} \text{ (leitura do giroscópio)} \\
%    {^{b}{\dot{\mathbf{v}}^{frd}_{cm/e}}}
%           &= \textstyle{\frac{1}{m}} {\mathbf{F}^{frd}} + {C_{frd/tp}}{\mathbf{g}^{tp}} - {\tilde{\mathbf{\omega}}^{frd}_{b/e}} {\mathbf{v}^{frd}_{cm/e}} \\
%    {\Phi} &= \begin{bmatrix} \phi & \theta & \psi \end{bmatrix}^{T} \text{ (calculado)} \\
%    \dot{\Phi}\left(\Phi,\omega \right) &=  H \left( \Phi \right) {\mathbf{\omega}^{frd}_{b/e}} \\
%    C_{frd/tp}\left(\Phi\right) &= fn \left( \Phi \right) \\
%    {^{e}{\mathbf{p}^{tp}_{cm/Q}}\left(t\right)} 
%        &= \int_{t_0}^{t}{^{e}{\dot{\mathbf{p}}^{tp}_{cm/Q}}\left(t\right)} \\
%    {\mathbf{v}^{frd}_{cm/e}\left(t\right)}
%        &= \int_{t_0}^{t}{^{b}{\dot{\mathbf{v}}^{frd}_{cm/e}}}{\left(t\right)}\textrm{dt}
%        = \int_{t_0}^{t}\begin{bmatrix} {\dot{U}} & {\dot{V}} & {\dot{W}} \end{bmatrix}^{T} \textrm{dt}\\
%    \int_{t_0}^{t}{^{e}{\dot{\mathbf{p}}^{tp}_{cm/Q}}\left(t\right)}
%        &= \int_{t_0}^{t}{C_{tp/frd}\left({\Phi}\right)}{\mathbf{v}^{frd}_{cm/e}\left(t\right)}\textrm{dt}
%        ={C_{tp/frd}\left({\Phi}\right)}\int_{t_0}^{t}\begin{bmatrix} {\dot{U}} & {\dot{V}} & {\dot{W}} \end{bmatrix}^{T} \textrm{dt} \\
%\end{align*}
