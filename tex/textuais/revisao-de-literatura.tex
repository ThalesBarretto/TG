...
Consideramos um corpo rígido (sistema \(F_{b}\)), tomando seu centro de masssa como ponto de referência para separar a dinâmica rotacional da translacional, e um sistema de coordenadas, \(bf\), que orresponde ao sistema \(frd\), fixo no corpo e com origem no centro de massa. A aceleração do centro de massa do corpo resulta da soma vetorial das forças, cujas linhas de ação não precisam passar pelo centro de massa (separamos o efeito de qualquer torsão para as equações do momento, das quais que não trataremos). As equações são expressas em termos de movimento relativo à Terra.

 O vetor posição pode partir de um ponto fixo arbitrário no quadro da Terra, \(F_{e}\). Se a variação da gravidade for significativa ao longo da trajetória, o vetor posição deve ser tomado a partir do centro de massa da Terra, mas para navegação de curto alcance pode ser mais conveniente tomá-lo a partir de um ponto na superfície da Terra.

O vetor velocidade será obtido em equações convenientes para navegação usando a velocidade do centro de massa do veículo em \(F_{e}\) e o vetor posição a partir do centro de massa da Terra. O centro de massa da Terra é um ponto fixo comum nos quadros inercial, \(F_{i}\),  e da Terra, \(F_{e}\), então a derivada de um vetor posição a partir do centro de massa fornece a velocidade inercial ou a velocidade terrestre, conforme o quadro em que derivamos. Derivadas em \(F_{i}\) e \(F_{e}\) são relacionadas ao vetor velocidade angular da Terra, \(\mathbf{\omega}_{e/i}\), de acordo com a equação de Coriolis. Além dos quadros acima, o quadro do corpo rígido, \(F_{b}\), também será necessário.
...
Desse modo, as equações de estado para posição e velocidade, em forma vetorial, são:
\begin{align*}\tag{1.7-12}
   {^{e}{\dot{\mathbf{p}}}_{cm/O}} = \mathbf{v}_{cm/e} \\
   {^{e}{\dot{\mathbf{v}}}_{cm/e}} = \mathbf{a}_{cm/e} \\
\end{align*}

Aplicando a segunda lei de Newton a um corpo rígido de massa constante próximo à superfície da Terra, para calcular a derivada inercial da velocidade sob influência de forças, partimos da Equação (1.5-5), substituímos \(\left( \textstyle{\frac{1}{m}}\mathbf{F} + \mathbf{G}\right)\) pela aceleração inercial, e isolamos a aceleração relativa (ou seja, a derivada do estado velocidade em \(F_{e}\)):
...
Esta equação de estado da velocidade combinada à equação de estado da posição são adequadas para a Terra oblata e girante. Latitude, longitude e \(\mathbf{G}\) podem ser calculadas a partir do vetor de posição geocêntrico. Já a força de Coriolis normalmente é desprezada para velocidades inferiores a 609m/s (2000 pés/s).

\section{Equações de Estado para Terra plana}
As equações de terra plana pressupõem que:
\begin{enumerate}
    ...
    \item A altura acima do plano tangente é uma boa aproximação para a altura verdadeira acima da superfície da Terra, e a projeção horizontal do vetor posição oferece uma boa aproximação à distância percorrida sobre a superfície da Terra (o que é razoável até algumas centenas de milhas a partir da origem do plano tangente).
...
Equações de Terra plana, embora não são sirvam para determinar com precisão a posição sobre a Terra, são amplamente utilizadas em simulações para estudar o desempenho e o comportamento dinâmico de veículos, e são empregadas para derivar modelos lineares de espaço de estados para estudos analíticos e projeto de sistemas de controle. 


Podemos simplificar as equações vetoriais (1.7-12) e (1.7-13) para obter as \emph{equações de Terra plana}. Considerando o vetor \(\mathbf{g}\) constante sobre a superfície da Terra, e o vetor posição a partir de um ponto de interesse \(Q\) na superfície, então a equação de estado para posição é

\begin{equation*}\tag{1.7-15}
   {^{e}{\dot{\mathbf{p}}_{cm/Q}}} = \mathbf{v}_{cm/e}
\end{equation*}

A equação da velocidade também pode ser formulada em relação ao veículo, com os componentes expressos num sistema de coordenadas fixo no corpo. Mudando as derivadas na equação (1.7--13), com \(\mathbf{G} \equiv \mathbf{g}\), temos
...
Um sistema \(frd\), fixo em \(F_{b}\), convém para o vetor velocidade em \(F_{b}\) e para o vetor velocidade angular do veículo (que utiliza os eixos do componentes em eixos do corpo nas equações de movimento angular), mas o vetor \(\mathbf{g}\) e  o vetor velocidade angular da Terra, conhecidos em sistemas fixos à Terra, devem ser rotacionados para o sistema do corpo, usando uma matriz rotacional que é obtida a partir das equações de estado. Nas equações de Terra plana a mudança de atitude do veículo será modelada com as equações cinemáticas de Euler (1.4-4),  relacionando o sistema \(frd\) fixo no corpo ao sistema \(ned\) (o qual  corresponde ao plano tangente \(tp\) fixo na Terra). Com os ângulos de Euler podemos obter a matriz de rotação \(C_{frd/ned}\), para então calcular as equações de posição e velocidade.

Para os nossos propósitos, o vetor de estado fica assimm:
% \(\mathbf{\omega}_{b/i} \equiv \mathbf{\omega}_{b/e}\)%:
...
Derivando as variáveis de estado, as relacionamos pelas equações\footnotemark{}:
...
\begin{align*}\tag{1.7-18}
    ...
    %{ ^{b}{\dot{\mathbf{\omega}}^{frd}_{b/e}}} &= {\left( J^{frd} \right)}^{-1} \begin{bmatrix} \mathbf{M}^{frd} & -{\tilde{\mathbf{\omega}}^{frd}_{b/e}} {J^{frd}} & {\mathbf{\omega}^{frd}_{b/e}}
    \end{bmatrix}
\end{align*}
...
\footnotetext{com o componente abaixo, \(\mathbf{g}_{D}\) igual à gravidade padrão (\(9.80665 \textrm{m/s}^{2}\)) ou seu valor local.}
